const dom = {
  ...
  metricCancelledCount: document.getElementById("metricCancelledCount"),
  ...
  listOrderState: document.getElementById("listOrderState"),
  listPrintState: document.getElementById("listPrintState"),
  copyVisibleButton: document.getElementById("copyVisibleButton"),
  exportVisibleButton: document.getElementById("exportVisibleButton"),
...
function getOrderPrimaryTimestamp(order) {
  return parseDateValue(order?.paidAtUtc)
    || parseDateValue(order?.createdAtUtc)
    || parseDateValue(order?.processedAtUtc)
    || parseDateValue(order?.updatedAtUtc);
}
...
function isTodayOrder(order) {
  const timestamp = getOrderPrimaryTimestamp(order);
  if (!timestamp) {
    return false;
  }
...
function getOrderPrintBlockReason(order) {
  if (order?.printedAtUtc) {
    return `已打印 ${order.printCount || 1} 次`;
  }
  ...
  if (!isAwaitingShipmentOrder(order)) {
    return `${translateStatus(order?.status)}，不在待发货`;
  }

  return "待打印";
}

function summarizeVisibleAmounts(orders) {
  ...
  for (const order of orders) {
    ...
    const currency = String(order?.currency || "").trim() || "未标币种";
    totals.set(currency, (totals.get(currency) || 0) + amount);
  }

  return [...totals.entries()]
    .map(([currency, total]) => `${formatQuantityValue(total)} ${currency}`)
    .join(" / ");
}

function buildVisibleOrdersText(orders) {
  const lines = [
    "订单号\t用户名\t金额\t状态\t支付时间"
  ];
  ...
  for (const order of orders) {
    lines.push([
      ...
      formatDate(order.paidAtUtc || order.createdAtUtc)
    ].join("\t"));
  }

  lines.push("");
  lines.push(`合计\t\t${summarizeVisibleAmounts(orders) || "0"}`);
  return lines.join("\r\n");
}

function buildVisibleOrdersCsv(orders) {
  ...
  const rows = [
    ...
    ["合计", "", summarizeVisibleAmounts(orders) || "0", "", ""]
  ];
...
async function copyVisibleOrders() {
  const orders = getVisibleOrders();
  if (!orders.length) {
    showToast("当前列表没有可复制的订单。", "error");
    return;
  }

  await copyText(buildVisibleOrdersText(orders), "当前列表已复制。");
}

function exportVisibleOrders() {
  const orders = getVisibleOrders();
  if (!orders.length) {
    showToast("当前列表没有可导出的订单。", "error");
    return;
  }

  const blob = new Blob([`\uFEFF${buildVisibleOrdersCsv(orders)}`], { type: "text/csv;charset=utf-8;" });
  const url = URL.createObjectURL(blob);
  const link = document.createElement("a");
  const dateLabel = new Date().toISOString().slice(0, 10);
  link.href = url;
  link.download = `orders-${dateLabel}.csv`;
  document.body.appendChild(link);
  link.click();
  link.remove();
  URL.revokeObjectURL(url);
  showToast("当前列表已导出。", "success");
}
...
function renderMetrics() {
  ...
  dom.metricCancelledCount.textContent = String(state.cachedOrders.filter((order) => isCancelledOrder(order)).length);
...
function matchesOrderState(order, orderState) {
  if (orderState === "all") {
    return true;
  }
  ...
  if (orderState === "awaiting") {
    return isAwaitingShipmentOrder(order) && isPaidOrder(order) && !isCancelledOrder(order);
  }

  if (orderState === "paid") {
    return isPaidOrder(order) && !isCancelledOrder(order);
  }
...
function getPrintBadge(order) {
  ...
  if (state.config?.autoPrintNewOrders && state.config?.autoPrintAfterBridgeCapture && isAwaitingShipmentOrder(order) && !order.buyerAccountName) {
...
function renderDeskMeta() {
  ...
  dom.deskMeta.textContent = "缓存订单由后台轮询自动同步。列表默认可按当日、已支付、未支付、已取消快速切换；已打印订单不会再走普通打印。";
}
...
function renderOrderCard(order) {
  ...
  const sourceDetail = order.buyerAccountNameSource ? "后台桥接" : "订单接口 / 未回填";
  const amountText = formatMoney(order.totalAmount, order.currency);
  const noteText = getOrderPrintBlockReason(order);
  ...
  const primaryActionLabel = canReprintOrder ? "重新打印" : "打印这单";
  const timeLabel = formatDateShort(order.paidAtUtc || order.createdAtUtc);

  return `
    <article class="order-card ${selected ? "is-selected" : ""} ${isCancelledOrder(order) ? "is-cancelled" : ""}" data-order-id="${escapeHtml(order.orderId)}">
      <div class="order-card-header order-card-header--compact">
        <label class="check-wrap">
          <input type="checkbox" data-select-order="${escapeHtml(order.orderId)}" ${selected ? "checked" : ""}>
          <span>选择</span>
        </label>

        <div class="order-card-main order-card-main--compact">
          ...
          <div class="order-card-handle">${escapeHtml(handleText)}</div>
          <div class="order-card-note">${escapeHtml(noteText)}</div>
        </div>
        ...
        <aside class="order-side-actions order-side-actions--compact">
          <button class="small-button" type="button" data-order-action="preview" data-order-id="${escapeHtml(order.orderId)}">查看详情</button>
          ${(canPrint || canReprintOrder)
            ? `<button class="small-button primary" type="button" data-order-action="${escapeHtml(primaryAction)}" data-order-id="${escapeHtml(order.orderId)}">${escapeHtml(primaryActionLabel)}</button>`
            : ""}
        </aside>
      </div>

      <div class="order-card-flags">
        ...
        <span class="source-badge ${order.buyerAccountNameSource ? "source-bridge" : ""}">${escapeHtml(sourceDetail)}</span>
      </div>
    </article>
  `;
}
...
function renderPreview() {
  ...
  const primaryActionLabel = preview.printedAtUtc ? "重新打印" : (canPrint ? "打印这单" : "当前不可打印");
  const primaryAction = preview.printedAtUtc ? "reprint" : "print";
    ...
    <section class="preview-section">
      <div class="preview-header">
        ...
        <div class="preview-actions">
          <button class="primary-button" type="button" data-preview-action="${escapeHtml(primaryAction)}" ${!preview.printedAtUtc && !canPrint ? "disabled" : ""}>${escapeHtml(primaryActionLabel)}</button>
          <button class="secondary-button" type="button" data-preview-action="reload">刷新预览</button>
...
function attachPreviewEvents() {
  ...
  dom.previewContent.querySelectorAll("[data-preview-action]").forEach((button) => {
    button.addEventListener("click", async () => {
      ...
      if (action === "reload") {
        await openPreview(preview.orderId, true);
      }
...
function setHistoryRange(days) {
  ...
  if (days === 0) {
    ...
    from.setTime(to.getTime() - days * 24 * 60 * 60 * 1000);
  }
...
function setActiveTab(tab) {
  state.activeTab = tab;
  if (tab === "history" && dom.listOrderState.value === "today") {
    dom.listOrderState.value = "all";
  }
  renderOrders();
}
...
async function printSelectedOrders() {
  ...
  try {
    ...
    showToast(`批量打印完成：成功 ${result.printed ?? 0} 单，跳过 ${failedCount} 单。`, failedCount ? "info" : "success");
    if (failedCount) {
      const detail = result.failed
        .slice(0, 5)
        .map((item) => `${item.orderId}：${item.message}`)
        .join("；");
      setMessage(`批量打印已跳过部分订单：${detail}`, "error");
    }
  } catch (error) {
...
dom.copyVisibleButton.addEventListener("click", () => {
  copyVisibleOrders().catch(() => {});
});
dom.exportVisibleButton.addEventListener("click", exportVisibleOrders);
...
dom.listOrderState.addEventListener("change", renderOrders);
...
dom.contextButtons.forEach((button) => {
  button.addEventListener("click", async () => {
    ...
    if (button.dataset.menuAction === "reprint") {
      await reprintOrder(order.orderId);
    }
